...
##PArallelize split
clusterExport(cl, varlist = c("mcl"))
clusterEvalQ(cl, {
  library(data.table)
})

process_row <- function(i) {
  cluster_id <- mcl$clusters[i]
  genes_split <- strsplit(mcl$X1[i], "\t")[[1]]
  temp_dt <- data.table(clusters = cluster_id, X1 = genes_split)
  return(temp_dt)
}

result_list <- parLapply(cl, 1:nrow(mcl), process_row)
mcl_melt <- rbindlist(result_list)
